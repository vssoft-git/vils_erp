...
#Область РаспределениеСредствКонтракта
	...
	ОтборОборотов.ТипИтогов =  Перечисления.ТипыИтогов.ОборотДт;
	ДобавитьОбороты(НовоеПравило, ОтборОборотов, ПараметрыОтчета.Детализация);
	// begin fix Suetin 19.11.2019 17:31:17
	ТеЗ = 
	"ВЫБРАТЬ
	...
	|ГДЕ
	|	НоменклатураДополнительныеРеквизиты.Свойство.Имя = ""ВидМатериальноПроизводственногоЗапаса""
	|	И НоменклатураДополнительныеРеквизиты.Значение = &Значение";
	ЗН = Новый Запрос(ТеЗ);
	ЗН.УстановитьПараметр("Значение", Справочники.ЗначенияСвойствОбъектов.НайтиПоНаименованию("Услуги сторонних организаций", Истина));
	МассивНоменклатурыЗН = Зн.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");
	// end fix Suetin 19.11.2019 17:31:22
	ОтборОборотов = ОтборОборотов();
	ОтборОборотов.Счет 							= СчетаУчетаПроизводство;
	ОтборОборотов.Субконто1 					= Перечисления.ТипыЗатратРегл.МатериальныеЗатраты;
	ОтборОборотов.ИсключаемыеОтборСубконто2		= МассивНоменклатурыЗН;	
	ОтборОборотов.НаправлениеДеятельности 		= "Контракт";
	ОтборОборотов.КорСчет 						= СчетаУчетаПроизводство;
	...
	//end fix Клещ А.Н. 24.10.2019
	ДобавитьОбороты(НовоеПравило, ОтборОборотов, ПараметрыОтчета.Детализация);
	// begin fix Suetin 19.11.2019 10:52:14
	ОтборОборотов = ОтборОборотов();
	МассивСчет20 = Новый Массив();
	МассивСчет20.Добавить(ПланыСчетов.Хозрасчетный.ОсновноеПроизводство);
	ОтборОборотов.Счет 							= МассивСчет20;
	ОтборОборотов.Субконто1 					= Перечисления.ТипыЗатратРегл.МатериальныеЗатраты;
	ОтборОборотов.Субконто2 					= МассивНоменклатурыЗН;
	ОтборОборотов.НаправлениеДеятельности 		= "Контракт";
	ОтборОборотов.КорСчет 						= МассивСчет20;
	ОтборОборотов.КорНаправлениеДеятельности 	= "";
	ОтборОборотов.ТипИтогов 					= Перечисления.ТипыИтогов.ОборотДт;
	ОтборОборотов.ИзменяетсяПодразделение 		= Ложь;
	//ОтборОборотов.ИсключаемыеОтборСубконто2ВидРасходов = МассивВидовРасходов;
	ДобавитьОбороты(НовоеПравило, ОтборОборотов, ПараметрыОтчета.Детализация);
	// end fix Suetin 19.11.2019 10:52:29
	ОтборОборотов = ОтборОборотов();
	ОтборОборотов.Счет = СчетаУчетаПроизводство;
	ТипыЗатрат = Новый Массив;
	...
	ОтборОборотов = ОтборОборотов();       										// begin fix Suetin 21.11.2019 10:24:14
	...
	ОтборОборотов.ТипИтогов = Перечисления.ТипыИтогов.ОборотДт;
	ОтборОборотов.НаправлениеДеятельности = "Контракт";
	ДобавитьОбороты(НовоеПравило, ОтборОборотов, ПараметрыОтчета.Детализация);	// end fix Suetin 21.11.2019 10:24:28
...
#Область ОтгрузкаПродукцииВыполнениеРабот
	...
	//ОтборОборотов = ОтборОборотов();       										// begin fix Suetin 21.11.2019 10:22:59 Перенесено в 2.3.5
	//ОтборОборотов.Счет = СчетаПроизводства();//FIX Клещ А.Н. СчетаУчетаКоммерческиеРасходы;
	//ОтборОборотов.КорСчет   = ПолучитьСчетКоммерческиеРасходы();
	//ОтборОборотов.ТипИтогов = Перечисления.ТипыИтогов.ОборотДт;
	//ОтборОборотов.НаправлениеДеятельности = "Контракт";
	...
	//ДобавитьОбороты(НовоеПравило, ОтборОборотов, ПараметрыОтчета.Детализация);    // end fix Suetin 21.11.2019 10:23:42
...
Процедура ВИЛС_ДобавитьОборот(Правило, ОтборОборота)
	...
	// begin fix Suetin 19.11.2019 12:42:39
	МассивСубконто2 = Новый Массив;
	Если ТипЗнч(ОтборОборота.Субконто2) = Тип("Массив") Тогда
		МассивСубконто2 = ОбщегоНазначенияКлиентСервер.СкопироватьМассив(ОтборОборота.Субконто2);
	ИначеЕсли ЗначениеЗаполнено(ОтборОборота.Субконто2) Тогда
		МассивСубконто2.Добавить(ОтборОборота.Субконто2);
	КонецЕсли;
	Оборот.Вставить("Субконто2", МассивСубконто2);
	
	МассивИсключаемыеСубконто2 = Новый Массив;
	Если ТипЗнч(ОтборОборота.ИсключаемыеОтборСубконто2) = Тип("Массив") Тогда
		МассивИсключаемыеСубконто2 = ОбщегоНазначенияКлиентСервер.СкопироватьМассив(ОтборОборота.ИсключаемыеОтборСубконто2);
	ИначеЕсли ЗначениеЗаполнено(ОтборОборота.ИсключаемыеОтборСубконто2) Тогда
		МассивИсключаемыеСубконто2.Добавить(ОтборОборота.ИсключаемыеОтборСубконто2);
	КонецЕсли;
	Оборот.Вставить("ИсключаемыеОтборСубконто2", МассивИсключаемыеСубконто2);
	// end fix Suetin 19.11.2019 12:42:43
...
Функция ВИЛС_ОтборОборотов()
	...
	//end fix Клещ А.Н. 24.10.2019
	// begin fix Suetin 19.11.2019 12:16:30
	ОтборОборота.Вставить("Субконто2");
	ОтборОборота.Вставить("ИсключаемыеОтборСубконто2");
	// end fix Suetin 19.11.2019 12:16:36
...
Функция ВИЛС_ТекстЗапросаПоПравилу(Правило, ПараметрыОтчета, Запрос)
	...
	//end fix Клещ А.Н. 24.10.2019
	// begin fix Suetin 19.11.2019 12:17:23
	|	И &Отбор_Субконто2
	|	И &Исключаемые_ОтборСубконто2
	// end fix Suetin 19.11.2019 12:17:29
	|
	|СГРУППИРОВАТЬ ПО
	...
	Для каждого Оборот Из Правило.Обороты Цикл
		...
		КонецЕсли;
		//end fix Клещ А.Н. 24.10.2019
		// begin fix Suetin 19.11.2019 12:17:59
		Если Не ЗначениеЗаполнено(Оборот.Значение.Субконто2) Тогда
			ТекстОборот = СтрЗаменить(ТекстОборот, "&Отбор_Субконто2", "ИСТИНА // Субконто2");
		Иначе
			Параметр = СтрШаблон("Субконто2_%1_%2", КодПоказателя, ИндексОборота);
			ОтборСубконто2 = СтрШаблон("ХозрасчетныйОбороты.Субконто2 В (&%1)", Параметр);
			ТекстОборот = СтрЗаменить(ТекстОборот, "&Отбор_Субконто2", ОтборСубконто2);
			Запрос.УстановитьПараметр(Параметр, Оборот.Значение.Субконто2);
		КонецЕсли;
		Если Не ЗначениеЗаполнено(Оборот.Значение.ИсключаемыеОтборСубконто2) Тогда
			ТекстОборот = СтрЗаменить(ТекстОборот, "&Исключаемые_ОтборСубконто2", "ИСТИНА // Исключаемые_ОтборСубконто2ВидРасходов");
		Иначе
			Параметр = СтрШаблон("Исключаемые_ОтборСубконто2_%1_%2", КодПоказателя, ИндексОборота);
			ИсключаемыеОтборСубконто2 = СтрШаблон("НЕ ХозрасчетныйОбороты.Субконто2 В (&%1)", Параметр);
			ТекстОборот = СтрЗаменить(ТекстОборот, "&Исключаемые_ОтборСубконто2", ИсключаемыеОтборСубконто2);
			Запрос.УстановитьПараметр(Параметр, Оборот.Значение.ИсключаемыеОтборСубконто2);
		КонецЕсли;
		// end fix Suetin 19.11.2019 12:18:07
