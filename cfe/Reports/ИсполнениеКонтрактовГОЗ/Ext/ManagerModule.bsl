...
Процедура ВИЛС_ДобавитьОборот(Правило, ОтборОборота)
	...
	КонецЕсли;
	Оборот.Вставить("ИсключаемыеОтборСубконто2ВидРасходов");
	//end fix Клещ А.Н. 24.10.2019
...
Функция ВИЛС_ТекстЗапросаПоПравилу(Правило, ПараметрыОтчета, Запрос)
	...
	Для каждого Оборот Из Правило.Обороты Цикл
		...
		//Если Оборот.Значение.КорНаправлениеДеятельности = Справочники.НаправленияДеятельности.ПустаяСсылка() Тогда
		//	Запрос.УстановитьПараметр("ОтборКорНаправлениеДеятельностиДоп", Оборот.Значение.ИсключаемыеОтборСубконто2ВидРасходов);
		//КонецЕсли;
...
Функция ВИЛС_ОтборКорНаправлениеДеятельности(ПараметрыОтчета, КорНаправлениеДеятельности)
	...
	Иначе
		...
		//	Отбор = "ХозрасчетныйОбороты.КорНаправлениеДеятельности В (&ОтборКорНаправлениеДеятельностиДоп)";
		//	//end fix Клещ А.Н. 29.10.2019
