...
// begin fix Suetin 30.04.2021 13:48:02
	...
	|					И РасчетыПоСрокам.ДатаВозникновения 		= РасчетыПредПредварительная.ДатаВозникновения
	|	{ГДЕ
	|		РасчетыПоСрокам.Период МЕЖДУ &НачалоВремен И &КонецПериода}     //НачалоВремен <- НачалоПериода
	|	
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	...
	|					И РасчетыПланОплат.РасчетныйДокумент 		= РасчетыПредПредварительная.РасчетныйДокумент
	...
	|					И РасчетыПланОтгрузок.РасчетныйДокумент 		= РасчетыПредПредварительная.РасчетныйДокумент
	...
	|					И РасчетыПредПредварительная.ДокументПлан 				= РасчетыПланОтгрузокОбороты.ДокументПлан
	...
	|					И РасчетыСПоставщиками.ОбъектРасчетов 			= РасчетыПредПредварительная.ОбъектРасчетов
