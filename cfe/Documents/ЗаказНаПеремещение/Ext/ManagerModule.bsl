...
Функция ВИЛС_СформироватьПечатнуюФормуЗаказНаПеремещение(МассивОбъектов, ОбъектыПечати)
	...
	"ВЫБРАТЬ
		...
		|	ДвиженияСерийТоваров.Документ КАК Документ,
		...
		|						ВТ.Номенклатура,
		|						ВТ.Характеристика,
		|						ВЫРАЗИТЬ(ВТ.Регистратор.Комментарий КАК СТРОКА(300))
		|					ИЗ
		...
		|		ПО (ВТ_СерииПриход.Номенклатура = ВТ_СерииПриход.Номенклатура)
