...
Функция ВИЛС_ТекстЗапросаДанныхШапкиДляПечатиСчетаФактуры(ПараметрыПечати)
	...
	|
#Вставка	
	|   ВЫБОР КОГДА НЕ ТипЗначения(ДанныеОснований.Ссылка) = Тип(Документ.ЗаписьКнигиПродаж) ТОГДА        // begin fix Suetin 05.02.2019 2:40:45
	|   	ДанныеОснований.Договор        //   .Ссылка
	|	ИНАЧЕ
	|		НЕОПРЕДЕЛЕНО
	|   КОНЕЦ КАК Договор,                                                                                // end fix Suetin 05.02.2019 2:40:46
#КонецВставки	
...
&ИзменениеИКонтроль("ПустойТекстЗапросаШапокОснований")
Функция ВИЛС_ПустойТекстЗапросаШапокОснований()
...
#Вставка
	...
	|	ДанныеДокументов.Ссылка.Договор                           КАК Договор,                               // end fix Suetin 02.09.2020 12:22:40
#КонецВставки
	|	ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка) КАК Комиссионер,
	|	НЕОПРЕДЕЛЕНО КАК Основание,
	|	ДАТАВРЕМЯ(1,1,1) КАК ОснованиеДата,
