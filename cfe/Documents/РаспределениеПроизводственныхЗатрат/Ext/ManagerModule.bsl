...
Функция ВИЛС_СформироватьАктИнвентаризацииМатериаловИПолуфабрикатов(ПараметрыПечати, ОбъектыПечати)
	...
	|ГДЕ
	|	ВЫБОР КОГДА ГруппыФУН.СчетУчетаНаСкладе = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Полуфабрикаты) 
	|			ТОГДА ИСТИНА   
	|		КОГДА ГруппыФУН.СчетУчетаНаСкладе = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ГотоваяПродукция)
	|			ТОГДА ИСТИНА
	|		КОГДА ГруппыФУН.СчетУчетаНаСкладе В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Материалы)) 
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ 
	|	КОНЕЦ
	|;
	...
	////Запрос.УстановитьПараметр("ОкончаниеПериодаГраница"	, ?(ПараметрыПечати.Период = Дата(1,1,1), Дата(1,1,1), (КонецМесяца(ПараметрыПечати.Период) + 1)));
	Запрос.УстановитьПараметр("ОкончаниеПериодаГраница"	, ?(ПараметрыПечати.Период = Дата(1,1,1), Дата(1,1,1), Новый Граница(КонецМесяца(ПараметрыПечати.Период), ВидГраницы.Включая)));
	//Запрос.УстановитьПараметр("БезОтбораПоПериоду"		, ПараметрыПечати.Период = Дата(1,1,1));   
	...
	// end fix Suetin 15.01.2020 17:26:30
