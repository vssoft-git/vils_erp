...
Функция ВИЛС_СформироватьАктИнвентаризацииМатериаловИПолуфабрикатов(ПараметрыПечати, ОбъектыПечати)
	...
	//Если ПараметрыПечати.свойство("НовоеПроизводство") Тогда
		...
		ТекстЗапроса = "
		    ...
		    |	СУММА(ДД.гРасходНаПроизводствоКоличество) КАК ,гРасходНаПроизводствоКоличество																	// fix Suetin 23.12.2019 13:04:26	Наличие,  гРасходНаПроизводствоКоличество
		    ...
		    |	СУММА(ДД.еКонечныйОстатокКоличество) КАК Наличие	//еКонечныйОстатокКоличество//,
	...
	|			ПО ДД.АналитикаУчетаНоменклатуры = Аналитика.Ссылка
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПартииПроизводства КАК СпрПартииПроизводства
	|			ПО (СпрПартииПроизводства.Ссылка = ДД.Партия)) КАК ДД
	...
	|ГДЕ (ВЫБОР КОГДА &ПодразделениеЗаполнено ТОГДА &Подразделение = ДД.Подразделение ИНАЧЕ ИСТИНА КОНЕЦ) 
	|СГРУППИРОВАТЬ ПО
	|	ДД.Организация,
	...
	|	СУММА(гРасходНаПроизводствоКоличество),
	|	СУММА(Наличие) 
	|ПО
	|	Организация,
	|	Подразделение,
