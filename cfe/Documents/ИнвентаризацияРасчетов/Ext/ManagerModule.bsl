...
Функция ПолучитьДанныеСтроки_180511(Знач ВыборкаШапка, Знач ВыборкаБезИерархии, Знач ВыборкаДокументов, Знач Выборка_3, Знач ДтКт = "Дт", Знач СуммаДолгаПоДоговору = 0, Знач Просроченная = Ложь, Знач ДанныеОДокПросроч = Неопределено) 
	...
	Для каждого ВыборкаДетальныеЗаписи из ТаблицаДокументовДолга цикл
		...
		ЗаполнитьСтрокуДаннымиДокумента(НовСтрока, ВыборкаДетальныеЗаписи.Регистратор,  ?(ЗначениеЗаполнено(ДоговорКонтрагента) и ТипЗнч(ДоговорКонтрагента) = Тип("СправочникСсылка.ДоговорыКонтрагентов"), ДоговорКонтрагента.ТипДоговора, ""), ВыборкаДетальныеЗаписи.ПроверятьГалочку, ВыборкаДетальныеЗаписи.ЕстьУслуги);
