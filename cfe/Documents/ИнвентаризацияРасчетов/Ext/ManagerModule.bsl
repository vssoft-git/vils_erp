...
Функция ПолучитьДанныеСтроки(Знач ВыборкаШапка, Знач ВыборкаБезИерархии, Знач ВыборкаДокументов, Знач ВыборкаОстатков, Знач Выборка_3, Знач ДтКт = "Дт", Знач СуммаДолгаПоДоговору = 0, Знач Просроченная = Ложь, Знач ДанныеОДокПросроч = Неопределено) 
	...
	Для Каждого ЭлСписка Из СписокОбъектовРасчета Цикл	  
		...
		Если КоличествоОбъектовРасчета > 1 Тогда
			...
			Если ЗначениеЗаполнено(ВыборкаБезИерархии.ДоговорКонтрагента) и ТипЗнч(ВыборкаБезИерархии.ДоговорКонтрагента) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
				Если ВыборкаБезИерархии.ДоговорКонтрагента.ВИЛС_УстановленСрокПоставкиПослеАванса          // КонтролироватьСрокПоставкиОтАванса - ВИЛС_УстановленСрокПоставкиПослеАванса
					и ВыборкаБезИерархии.СчетРасчетов = ПланыСчетов.Хозрасчетный.РасчетыСПокупателями Тогда
					МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученным);
					СтруктураПоиска = Новый Структура("ДебиторКредитор, ВИЛС_Договор, Счет, ОбъектРасчетов", ВыборкаБезИерархии.Контрагент, ВыборкаБезИерархии.ДоговорКонтрагента, ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученным, ОбъектРасчетов);
					СписокСтрокПоТекущейЗадолженности1 = ВыборкаДокументов.НайтиСтроки(СтруктураПоиска);
				ИначеЕсли ВыборкаБезИерархии.ДоговорКонтрагента.ВИЛС_УстановленСрокПоставкиПослеАванса          // КонтролироватьСрокПоставкиОтАванса - ВИЛС_УстановленСрокПоставкиПослеАванса
					и ВыборкаБезИерархии.СчетРасчетов = ПланыСчетов.Хозрасчетный.РасчетыСПоставщиками Тогда
					МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоАвансамВыданным);
					СтруктураПоиска = Новый Структура("ДебиторКредитор, ВИЛС_Договор, Счет, ОбъектРасчетов", ВыборкаБезИерархии.Контрагент, ВыборкаБезИерархии.ДоговорКонтрагента, ПланыСчетов.Хозрасчетный.РасчетыПоАвансамВыданным, ОбъектРасчетов);
					СписокСтрокПоТекущейЗадолженности1 = ВыборкаДокументов.НайтиСтроки(СтруктураПоиска);
				КонецЕсли; 
			КонецЕсли;
			//СтруктураПоиска = Новый Структура("ДебиторКредитор, ВИЛС_Договор, Счет", ВыборкаЗадолженность.Контрагент, ВыборкаЗадолженность.ДоговорКонтрагента, МассивСчетов);
			СтруктураПоиска = Новый Структура("ДебиторКредитор, ВИЛС_Договор, Счет, ОбъектРасчетов", ВыборкаБезИерархии.Контрагент, ВыборкаБезИерархии.ДоговорКонтрагента, ВыборкаБезИерархии.СчетРасчетов, ОбъектРасчетов);
			СписокСтрокПоТекущейЗадолженности = ВыборкаДокументов.НайтиСтроки(СтруктураПоиска);
			Если СписокСтрокПоТекущейЗадолженности1.Количество() Тогда
				Для Каждого СтрокаТЗТЗ Из СписокСтрокПоТекущейЗадолженности1 Цикл
					Если СписокСтрокПоТекущейЗадолженности.Найти(СтрокаТЗТЗ) = Неопределено Тогда
			...
			СуммаДолгаПоДоговору = ПолучитьОстаток(Счет, Контрагент, ДоговорКонтрагента, ОбъектРасчетов, ВыборкаОстатков, ДтКт);
		...
		Для каждого ВыборкаДетальныеЗаписи из ТаблицаДокументовДолга цикл
			...
			ЗаполнитьСтрокуДаннымиДокумента(НовСтрока, ВыборкаДетальныеЗаписи.Регистратор, ОбъектРасчетов, ТекущийОбъектРасчетов
				//, ?(ЗначениеЗаполнено(ОбъектРасчетов) и (ТипЗнч(ОбъектРасчетов) = Тип("СправочникСсылка.ДоговорыКонтрагентов") или ТипЗнч(ОбъектРасчетов) = Тип("СправочникСсылка.ДоговорыМеждуОрганизациями")), ОбъектРасчетов.СтатьяДвиженияДенежныхСредств, 
				//?(ТекущийОбъектРасчетов = ОбъектРасчетов, "", ОбъектРасчетов))
	...
	СтруктураСтроки.Вставить("ЗаЧто"					, ?(ЗначениеЗаполнено(ДоговорКонтрагента) и ТипЗнч(ДоговорКонтрагента) = Тип("СправочникСсылка.ДоговорыКонтрагентов"), ДоговорКонтрагента.СтатьяДвиженияДенежныхСредств, ""));
...
Процедура ЗаполнитьСтрокуДаннымиДокумента(НовСтрока, Знач Регистратор, Знач ОбъектРасчетов, Знач ТекущийОбъектРасчетов, Знач ПроверятьГалочку, Знач ЕстьУслуги = Ложь)
	НовСтрока.ЗаЧто 			= ?(ЗначениеЗаполнено(ОбъектРасчетов) 
									и (ТипЗнч(ОбъектРасчетов) = Тип("СправочникСсылка.ДоговорыКонтрагентов") 
										или ТипЗнч(ОбъектРасчетов) = Тип("СправочникСсылка.ДоговорыМеждуОрганизациями")), ОбъектРасчетов.СтатьяДвиженияДенежныхСредств, 
									?(ТекущийОбъектРасчетов = ОбъектРасчетов, "", ОбъектРасчетов));
...
Функция ПолучитьДанныеСтрокиДляПросроченной_2(Знач ВыборкаШапка, Знач ВыборкаБезИерархии, Знач ВыборкаДокументов, Знач ВыборкаОстатков, Знач Выборка_3, Знач ДтКт = "Дт", Знач СуммаДолгаПоДоговору = 0, Знач ДанныеОДокПросроч = Неопределено) 
	...
	Для Каждого ЭлСписка Из СписокОбъектовРасчета Цикл	  
		...
		Если КоличествоОбъектовРасчета > 1 Тогда
			...
			Если ЗначениеЗаполнено(ДоговорКонтрагента) и ТипЗнч(ДоговорКонтрагента) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
				Если ДоговорКонтрагента.ВИЛС_УстановленСрокПоставкиПослеАванса          // КонтролироватьСрокПоставкиОтАванса - ВИЛС_УстановленСрокПоставкиПослеАванса
					и Счет = ПланыСчетов.Хозрасчетный.РасчетыСПокупателями Тогда
					МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученным);
					СтруктураПоиска = Новый Структура("ДебиторКредитор, ВИЛС_Договор, Счет, ОбъектРасчетов", Контрагент, ДоговорКонтрагента, ПланыСчетов.Хозрасчетный.РасчетыПоАвансамПолученным, ОбъектРасчетов);
					СписокСтрокПоТекущейЗадолженности1 = ВыборкаДокументов.НайтиСтроки(СтруктураПоиска);
				ИначеЕсли ДоговорКонтрагента.ВИЛС_УстановленСрокПоставкиПослеАванса          // КонтролироватьСрокПоставкиОтАванса - ВИЛС_УстановленСрокПоставкиПослеАванса
					и Счет = ПланыСчетов.Хозрасчетный.РасчетыСПоставщиками Тогда
					МассивСчетов.Добавить(ПланыСчетов.Хозрасчетный.РасчетыПоАвансамВыданным);
					СтруктураПоиска = Новый Структура("ДебиторКредитор, ВИЛС_Договор, Счет, ОбъектРасчетов", Контрагент, ДоговорКонтрагента, ПланыСчетов.Хозрасчетный.РасчетыПоАвансамВыданным, ОбъектРасчетов);
					СписокСтрокПоТекущейЗадолженности1 = ВыборкаДокументов.НайтиСтроки(СтруктураПоиска);
				КонецЕсли; 
			КонецЕсли;
			//СтруктураПоиска = Новый Структура("ДебиторКредитор, ВИЛС_Договор, Счет", ВыборкаЗадолженность.Контрагент, ВыборкаЗадолженность.ДоговорКонтрагента, МассивСчетов);
			СтруктураПоиска = Новый Структура("ДебиторКредитор, ВИЛС_Договор, Счет, ОбъектРасчетов", Контрагент, ДоговорКонтрагента, Счет, ОбъектРасчетов);
			СписокСтрокПоТекущейЗадолженности = ВыборкаДокументов.НайтиСтроки(СтруктураПоиска);
			Если СписокСтрокПоТекущейЗадолженности1.Количество() Тогда
				Для Каждого СтрокаТЗТЗ Из СписокСтрокПоТекущейЗадолженности1 Цикл
					Если СписокСтрокПоТекущейЗадолженности.Найти(СтрокаТЗТЗ) = Неопределено Тогда
		...
		Для каждого ВыборкаДетальныеЗаписи из ТаблицаДокументовДолга цикл
			...
			СтруктПолуч = ПолучитьСтруктуруДанныхДокумента(ВыборкаДетальныеЗаписи.Регистратор  
							//?(ЗначениеЗаполнено(ДоговорКонтрагента) и ТипЗнч(ДоговорКонтрагента) = Тип("СправочникСсылка.ДоговорыКонтрагентов"), ДоговорКонтрагента.СтатьяДвиженияДенежныхСредств, ""), 
							, ОбъектРасчетов, ТекущийОбъектРасчетов
							, ВыборкаДетальныеЗаписи.ПроверятьГалочку, ДоговорКонтрагента, ВыборкаДетальныеЗаписи.ЕстьУслуги);
	...
	СтруктураСтроки.Вставить("ДатаДок", 		ДатаДок);
	СтруктураСтроки.Вставить("НомерДок", 		НомерДок);
	СтруктураСтроки.Вставить("ЗаЧто", 			?(ЗначениеЗаполнено(ДоговорКонтрагента) и (ТипЗнч(ДоговорКонтрагента) = Тип("СправочникСсылка.ДоговорыКонтрагентов") или ТипЗнч(ДоговорКонтрагента) = Тип("СправочникСсылка.ДоговорыМеждуОрганизациями")), ДоговорКонтрагента.СтатьяДвиженияДенежныхСредств, ЗаЧто));
	...
	СтруктураСтроки.Вставить("ЗаЧто1М", 					?(ЗначениеЗаполнено(ДоговорКонтрагента) и (ТипЗнч(ДоговорКонтрагента) = Тип("СправочникСсылка.ДоговорыКонтрагентов") или ТипЗнч(ДоговорКонтрагента) = Тип("СправочникСсылка.ДоговорыМеждуОрганизациями")), ДоговорКонтрагента.СтатьяДвиженияДенежныхСредств, ЗаЧто1М));
...
Функция ПолучитьСтруктуруДанныхДокумента(Регистратор, Знач ОбъектРасчетов, Знач ТекущийОбъектРасчетов, Знач ПроверятьГалочку, Знач ДоговорКонтрагента, Знач ЕстьУслуги = Ложь)
	НовСтрока = Новый Структура("ТипДокумента, ДатаДок, ДатаДокСист, НомерДок, ЗаЧто, ОбъектРасчетов, ПроверятьГалочку, ДатаПогашенияДЗ, ДатаПогашенияДЗСтр"); 
	ЗаполнитьСтрокуДаннымиДокумента(НовСтрока, Регистратор, ОбъектРасчетов, ТекущийОбъектРасчетов, ПроверятьГалочку, ЕстьУслуги);
	ЗаполнитьСтрокуДаннымиДоговора(НовСтрока, ДоговорКонтрагента);
	Возврат(НовСтрока);
КонецФункции
