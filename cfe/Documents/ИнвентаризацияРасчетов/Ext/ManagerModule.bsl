...
Функция ПечатьРеестрДебиторскойЗадолженностиПриложение_7_1_180510(МассивОбъектов, ОбъектыПечати)
	...
	Для каждого Ссылка Из МассивОбъектов Цикл
		...
		Для каждого СтрокаТЗ из СтрокиТЗ цикл
			...
			ДанныеОДок = ПолучитьДанныеСтроки_180511(новый граница(ДатаДокументаИнветаризации, ВидГраницы.Включая), Новый граница(ДатаДокументаИнветаризации, ВидГраницы.Включая), СтрокаТЗ.Контрагент, СтрокаТЗ.СчетРасчетов, ВыборкаШапка.Организация, СтрокаТЗ.ДоговорКонтрагента, "Дт", (СтрокаТЗ.Подтверждено + СтрокаТЗ.НеПодтверждено));
			...
			СуммаДЗБезРезерва 							= СтрокаТЗ.Подтверждено + СтрокаТЗ.НеПодтверждено;
...
Функция ПолучитьДанныеСтроки_180511(Знач КонПериода, Знач КонПериода1, Знач Контрагент, Знач Счет, Знач Организация, Знач ДоговорКонтрагента, Знач ДтКт = "Дт", Знач Задолженность, Знач Просроченная = Ложь, Знач ДанныеОДокПросроч = Неопределено)
	...
	//Иначе	
		...
		|	ИЗ
		|		Документ.ПриобретениеТоваровУслуг.ВидыЗапасов
		|	ОБЪЕДИНИТЬ
		|	ВЫБРАТЬ
		|		Т.Ссылка КАК Регистратор,
		|   	ВЫБОР 
		...
		|	ХозрасчетныйОбороты.ВИЛС_Договор, // end fix Suetin 17.01.2019 12:58:17
		
		|	ХозрасчетныйОбороты.Регистратор,
		|	ХозрасчетныйОбороты.Период,
		|	ХозрасчетныйОбороты.Счет,
		|	ХозрасчетныйОбороты.ПроверятьГалочку, 
		|	ХозрасчетныйОбороты.ЕстьУлуги
		|
		//|УПОРЯДОЧИТЬ ПО
		//|	ХозрасчетныйОбороты.Период
		|ИТОГИ
	...
	Запрос.УстановитьПараметр("ВТБ_Факторинг", 		Справочники.Контрагенты.НайтиПоКоду("00-00000004"));
	Запрос.УстановитьПараметр("Транскапиталбанк",	Справочники.Контрагенты.НайтиПоКоду("00-00000004"));   
	...
	Для каждого ВыборкаДетальныеЗаписи из ТаблицаДокументовДолга цикл
		...
		ЗаполнитьСтрокуДаннымиДокумента(НовСтрока, ВыборкаДетальныеЗаписи.Регистратор, ВыборкаДетальныеЗаписи.ПроверятьГалочку);
		
		НовСтрока.ДатаПогашенияДЗ = Дата(1,1,1);
		Если НовСтрока.ДатаДок = "б/д" Тогда
		    НовСтрока.ДатаПогашенияДЗСтр = "б/д";
		ИначеЕсли ДоговорКонтрагента.ВИЛС_УстановленСрокПоставкиПослеАванса и НовСтрока.ПроверятьГалочку = "Деньги" Тогда         // КонтролироватьСрокПоставкиОтАванса - ВИЛС_УстановленСрокПоставкиПослеАванса
		...
		ИначеЕсли ДоговорКонтрагента.ВИЛС_УстановленСрокПоставкиПослеАванса и НовСтрока.ПроверятьГалочку = "Поставка" Тогда         // КонтролироватьСрокПоставкиОтАванса - ВИЛС_УстановленСрокПоставкиПослеАванса
			//Если ЗначениеЗаполнено(ДоговорКонтрагента.ВИЛС_СрокПоставкиПослеАванса)  Тогда      // СрокПоставкиОтАванса - ВИЛС_СрокПоставкиПослеАванса
			//	НовСтрока.ДатаПогашенияДЗ = НовСтрока.ДатаДок + ДоговорКонтрагента.ВИЛС_СрокПоставкиПослеАванса * 60 * 60 * 24;// СрокПоставкиОтАванса - ВИЛС_СрокПоставкиПослеАванса
			//	НовСтрока.ДатаПогашенияДЗСтр = Формат(НовСтрока.ДатаПогашенияДЗ, "ДФ=dd.MM.yyyy");
			//Иначе
		...
		ИначеЕсли ДоговорКонтрагента.УстановленСрокОплаты и НовСтрока.ПроверятьГалочку = "Поставка"  Тогда   	// КонтролироватьСрокОкончательнойОплатыЗадолженности - УстановленСрокОплаты
	...
	СтруктураСтроки.Вставить("ИсполняющееПодразделение", ДоговорКонтрагента.Подразделение);    //  ИсполняющееПодразделение
