...
Процедура ВыбратьЭлементыБезИерархии(ВыборкаБезИерархии, ТаблицаСтрокПоСчету, ВыборкаДокументов, Выборка_3, ВыборкаШапка, ДтКт, ТабДокумент, Макет, СтруктураПоказателей, КоличествоВСтеке)     //ВыборкаОстатков, 
	Пока ВыборкаБезИерархии.Следующий() Цикл
		...
		ДанныеОДок = ПолучитьДанныеСтроки_180511(СписокСтрокПоТекущейЗадолженности, Выборка_3, ВыборкаШапка.КонецПериода, ВыборкаБезИерархии.Контрагент, ВыборкаБезИерархии.СчетРасчетов, ВыборкаШапка.Организация, ВыборкаБезИерархии.ДоговорКонтрагента, ДтКт, СуммаДЗБезРезерва); //МассивСтрокОстатковПоДоговору, 
...
Функция ПолучитьДанныеСтроки_180511(Знач СписокСтрокПоТекущейЗадолженности, Знач Выборка_3, Знач КонПериода, Знач Контрагент, Знач Счет, Знач Организация, Знач ДоговорКонтрагента, Знач ДтКт = "Дт", Знач СуммаДолгаПоДоговору = 0, Знач Просроченная = Ложь, Знач ДанныеОДокПросроч = Неопределено)    //Знач МассивСтрокОстатковПоДоговору, 
	...
	Для каждого СтрокаТЗИ Из СписокСтрокПоТекущейЗадолженности Цикл
		...
		////Прервать;
	КонецЦикла;	
	//Если МассивСтрокОстатковПоДоговору.Количество() Тогда
	//	СуммаДолгаПоДоговору =  ?(МассивСтрокОстатковПоДоговору[0].СуммаОстаток = 0, Задолженность, ?(ДтКт = "Дт", МассивСтрокОстатковПоДоговору[0].СуммаОстаток, - МассивСтрокОстатковПоДоговору[0].СуммаОстаток));
	//КонецЕсли;	
	//СуммаДолгаПоДоговору = Задолженность;
