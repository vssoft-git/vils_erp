...
Процедура ВИЛС_ОбновитьСервер()
	...
	Иначе
		...
		|	ЛОЖЬ КАК Оплачена,
		|	1 КАК ИндексКартинки,
		|	ЕСТЬNULL(ЗаказыКлиентовОбороты.СуммаПриход, 0) КАК СуммаПлатежа,
		|	ЕСТЬNULL(ЗаказыКлиентовОбороты.СуммаПриход, 0) КАК СуммаКОплате,
		|	&ТекущаяДата КАК ДатаПлатежа,
		|	0 КАК ПроцентПлатежа,
		|	ЛОЖЬ КАК ЭтоЗалогЗаТару,
		|	ЛОЖЬ КАК ЭтапСверхЗаказа
		|ИЗ
		|	РегистрНакопления.ЗаказыКлиентов.Обороты(, , Период, ЗаказКлиента = &ДокументОснование) КАК ЗаказыКлиентовОбороты
		|ГДЕ
		|	ЗаказыКлиентовОбороты.ЗаказКлиента.Договор.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоНакладным)
		|	И ЕСТЬNULL(ЗаказыКлиентовОбороты.СуммаПриход, 0) > 0
		|	И &ИспользоватьРасширенныеВозможностиЗаказаКлиента
		|
