...
Функция ВИЛС_СформироватьПечатнуюФормуМ2(МассивОбъектов, ОбъектыПечати)
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(Задание.Водитель.Партнер.Пол, ЗНАЧЕНИЕ(Перечисление.ПолФизическогоЛица.Женский)) = ЗНАЧЕНИЕ(Перечисление.ПолФизическогоЛица.Женский)
	|			ТОГДА 2
	|		ИНАЧЕ 1
	...
	|			И (ФИОФизическихЛиц.Период = Задание.ПериодРуководителя)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Задание
	|;
	...
	Пока ВыборкаШапкаДокументов.Следующий() Цикл
		...
		Уд = Найти(ВыборкаШапкаДокументов.ДокУдостоверяющийЛичность, "№ подр");
		ОбластьМакета.Параметры.ДокУдостоверяющийЛичность = Лев(ВыборкаШапкаДокументов.ДокУдостоверяющийЛичность,Уд-3);
...
//Функция ДатаПрописью(Знач ФорматируемаяДата)
//	Возврат(ЧислоПрописью(День(ФорматируемаяДата), "Л = ru_RU; НД = Ложь") + " " + Формат(ФорматируемаяДата, "ДФ=ММММ") + " " + ЧислоПрописью(Год(ФорматируемаяДата)));
//КонецФункции	
