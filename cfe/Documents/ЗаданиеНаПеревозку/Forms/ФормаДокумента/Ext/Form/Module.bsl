...
Процедура ВИЛС_МаршрутПередНачаломИзмененияПеред(Элемент, Отказ)
	Если Элемент.ТекущийЭлемент.Имя = "МаршрутАдрес"
		...
		Если МассивСтрокРаспоряжений.Количество() Тогда
			ТекущаяСтрокаРаспоряжений = МассивСтрокРаспоряжений[0];
			Если ТипЗнч(ТекущаяСтрокаРаспоряжений.ПолучательОтправитель) = Тип("СправочникСсылка.Партнеры") Тогда 
				ВИЛС_МаршрутАдресНачалоВыбораИзСпискаПередНаСервере(ТекущаяСтрокаРаспоряжений.ПолучательОтправитель);
			КонецЕсли;	
			//Если Элементы.МаршрутАдрес.СписокВыбора.Количество() Тогда
			//	ДополнительныеПараметры = Новый Структура("ТекущиеДанные", ТекущиеДанные);
			//	ОписаниеОповещенияПриЗакрытии = Новый ОписаниеОповещения("ВИЛС_МаршрутПриНачалеРедактированияПеред_Окончание", ЭтотОбъект, ДополнительныеПараметры); 
			//	ПоказатьВыборИзСписка(ОписаниеОповещенияПриЗакрытии, Элементы.МаршрутАдрес.СписокВыбора, Элементы.МаршрутАдрес);
			//КонецЕсли;
		КонецЕсли;	
	КонецЕсли;		
КонецПроцедуры
...
&НаСервере
Процедура ВИЛС_МаршрутАдресНачалоВыбораИзСпискаПередНаСервере(Знач Контрагент)
	ДоставкаТоваров.ВИЛС_ЗаполнитьСписокВыбораАдресовКонтрагентаПоКонтрагенту(Элементы, Контрагент, "МаршрутАдрес"); 
КонецПроцедуры
