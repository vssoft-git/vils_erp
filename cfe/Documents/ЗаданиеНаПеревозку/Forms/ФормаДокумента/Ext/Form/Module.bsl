...
Процедура ВИЛС_ПереопределитьРеквизиты()
	...
	РеквизитФормы = Новый РеквизитФормы("Вес", ДопустимыеТипы, "ТоварыКДоставке", "Вес");
	МассивРеквизитов.Добавить(РеквизитФормы);
	//Колонка = ТЗ.Колонки.Добавить("Объем", ДопустимыеТипы);
	//РеквизитФормы = Новый РеквизитФормы(Колонка.Имя, Колонка.ТипЗначения, "ТоварыКДоставке", Колонка.Заголовок);
	РеквизитФормы = Новый РеквизитФормы("Объем", ДопустимыеТипы, "ТоварыКДоставке", "Объем");
	МассивРеквизитов.Добавить(РеквизитФормы);
	...
	РеквизитФормы = Новый РеквизитФормы("ВИЛС_КонтактныеЛица", ДопустимыеТипы, "Объект.Маршрут", "Контактные лица", Ложь);
	МассивРеквизитов.Добавить(РеквизитФормы);
	
	РеквизитФормы = Новый РеквизитФормы("ВИЛС_ТекстТТН", ДопустимыеТипы, "", "Оформить ТТН", Ложь);
	МассивРеквизитов.Добавить(РеквизитФормы);
	...
	ДобавляемыйЭлементГрСВ = Элементы.Вставить("Группа_ВИЛС_ТекстТТН", Тип("ГруппаФормы"), Элементы.ГруппаВремяФакт, Элементы.ГруппаВремяРеквизитыТТН);	
	ДобавляемыйЭлементГрСВ.Вид 					= ВидГруппыФормы.ОбычнаяГруппа;
	ДобавляемыйЭлементГрСВ.Отображение 			= ОтображениеОбычнойГруппы.Нет;
	ДобавляемыйЭлементГрСВ.ОтображатьЗаголовок  = Ложь;
	ДобавляемыйЭлементГрСВ.СквозноеВыравнивание = СквозноеВыравнивание.Использовать;
	...
	ДобавляемыйЭлемент = Элементы.Вставить("ВИЛС_ТекстТТН", Тип("ПолеФормы"), ДобавляемыйЭлементГрСВ);
	ДобавляемыйЭлемент.Вид 						= ВидПоляФормы.ПолеНадписи; 
	ДобавляемыйЭлемент.ПутьКДанным				= "ВИЛС_ТекстТТН";
	ДобавляемыйЭлемент.Гиперссылка		 		= Истина;
	ДобавляемыйЭлемент.Видимость				= Истина;
	ДобавляемыйЭлемент.Заголовок				= "";
	ДобавляемыйЭлемент.АвтоМаксимальнаяШирина 	= Истина;
	ДобавляемыйЭлемент.УстановитьДействие("ОбработкаНавигационнойСсылки", "ВИЛС_ТекстТТНОбработкаНавигационнойСсылки");
КонецПроцедуры
...
Процедура ВИЛС_ЗаполнитьСлужебныеРеквизитыМаршрута(ОбновляемыеСтроки)
	...
	|	Распоряжения.Перевозчик,
	|	Распоряжения.ПолучательОтправитель,      
	|	Распоряжения.ВИЛС_КонтактноеЛицо,                 	// fix Suetin 30.08.2019 16:09:36
	...
	|	МАКСИМУМ(РаспоряженияПоМаршрутам.ВИЛС_КонтактноеЛицо) КАК ВИЛС_КонтактноеЛицо,                 // fix Suetin 30.08.2019 16:11:41
	...
	Пока Результат.СледующийПоЗначениюПоля("НомерСтрокиМаршрута") Цикл
		...
		Пока Результат.СледующийПоЗначениюПоля("Перевозчик") Цикл
			...
			Пока Результат.СледующийПоЗначениюПоля("ПолучательОтправитель") Цикл
				...
				КонтактныеЛицаПоПеревозчику = КонтактныеЛицаПоПеревозчику + Результат.ВИЛС_КонтактноеЛицо + "," + " ";    	// fix Suetin 30.08.2019 16:37:16
			КонецЦикла;
...
&НаКлиенте
Процедура ВИЛС_ТекстТТНОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылка, СтандартнаяОбработка)
