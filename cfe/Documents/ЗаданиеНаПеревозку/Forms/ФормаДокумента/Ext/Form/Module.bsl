...
Процедура ВИЛС_МаршрутАдресНачалоВыбораИзСпискаПередНаСервере(Знач ПараметрыОтбора)
	...
	Если МассивСтрокРаспоряжений.Количество() Тогда
		...
		Если ТипЗнч(ТекущаяСтрокаРаспоряжений.ПолучательОтправитель) = Тип("СправочникСсылка.Партнеры") 
			...
			ДоставкаТоваров.ВИЛС_ЗаполнитьСписокВыбораАдресовКонтрагентаПартнера(Элементы, ТекущаяСтрокаРаспоряжений.ПолучательОтправитель, "МаршрутАдрес");
		КонецЕсли;
		Если ТипЗнч(ТекущаяСтрокаРаспоряжений.Распоряжение) = Тип("ДокументСсылка.ЗаказПоставщику")
			...
			ДоставкаТоваров.ВИЛС_ЗаполнитьСписокВыбораАдресовКонтрагентаПартнера(Элементы, ТекущаяСтрокаРаспоряжений.Распоряжение.Контрагент, "МаршрутАдрес", Ложь);
		КонецЕсли;
	КонецЕсли; 
КонецПроцедуры
