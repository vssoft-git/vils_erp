...
Функция ВИЛС_ТекстЗапросаДанныеДокумента(НастройкиПечати)
	...
	|
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТЧГруппировкиФинИнструментов.Ссылка КАК Ссылка,
	|	ТЧГруппировкиФинИнструментов.Валюта КАК Валюта,
	|	ДатыКурсовВалют.Период КАК ДатаКурса,
	|	КурсыВалют.Курс КАК Курс,
	|	КурсыВалют.Кратность КАК Кратность
	|ИЗ
	|	Документ.СверкаВзаиморасчетов.ГруппировкиФинансовыеИнструменты КАК ТЧГруппировкиФинИнструментов
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.КурсыВалют КАК КурсыВалют
	|		ПО ТЧГруппировкиФинИнструментов.Валюта = КурсыВалют.Валюта
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ ДатыКурсовВалют КАК ДатыКурсовВалют
	|		ПО КурсыВалют.Валюта = ДатыКурсовВалют.Валюта
	|		И КурсыВалют.Период = ДатыКурсовВалют.Период
	|ГДЕ
	|	ТЧГруппировкиФинИнструментов.Ссылка В(&МассивОбъектов)
	|	
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка,
	|	Валюта
...
#КонецВставки
	...
	|			И ТЧГруппировкиРасчеты.ТипРасчетов = ТЧДетальныеЗаписиРасчеты.ТипРасчетов
	|ГДЕ
	...
	|	СУММА(ВЫРАЗИТЬ(ДетальныеЗаписи.КонечноеСальдоДт * КурсыВалют.Курс / КурсыВалют.Кратность КАК ЧИСЛО(31,2))) КАК КонечноеСальдоРеглДт,
	|	СУММА(ВЫРАЗИТЬ(ДетальныеЗаписи.КонечноеСальдоКт * КурсыВалют.Курс / КурсыВалют.Кратность КАК ЧИСЛО(31,2))) КАК КонечноеСальдоРеглКт,
	|	
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ КурсыВалют КАК КурсыВалют
	|	ПО ДетальныеЗаписи.ДокументСсылка = КурсыВалют.Ссылка
	|		И ДетальныеЗаписи.Валюта = КурсыВалют.Валюта
	|
	|СГРУППИРОВАТЬ ПО
