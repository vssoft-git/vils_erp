...
Процедура ВИЛС_ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры)
	...
	|	ДетальныеЗаписиРасчетов.РасчетныйДокумент КАК РасчетныйДокумент,
#Вставка
	|	ДетальныеЗаписиРасчетов.ВИЛС_РасчетныйДокумент КАК ВИЛС_РасчетныйДокумент,
#КонецВставки
	|	ДетальныеЗаписиРасчетов.ОписаниеДокумента КАК ОписаниеДокумента,
...
&ИзменениеИКонтроль("СвернутьРасчетыПоГруппировкам")
Процедура ВИЛС_СвернутьРасчетыПоГруппировкам(ДанныеДокумента, ТаблицаВзаиморасчетов)
...
#Вставка
			Если ТЧДетальныеЗаписи = ДанныеДокумента.ДетальныеЗаписиРасчеты Тогда
				НоваяСтрока.ВИЛС_РасчетныйДокумент = СтрокаРасчетов.РасчетныйДокумент;
			КонецЕсли; 
#КонецВставки
			//++ Локализация
			...
			Если СФДокумента.Количество() > 0 Тогда
				...
				Для Каждого СФ Из СФДокумента Цикл
					...
					КонецЕсли;
					ОписаниеСФ = СтрШаблон(НСтр("ru = 'Счет-фактура №%1 от %2';
					|en = 'Tax invoice №%1 from %2'"), НомерНаПечать, Формат(СФ.Дата, "ДЛФ=D"));
...
#КонецУдаления
#Вставка
	ДетальныеЗаписи.Свернуть("ТипРасчетов, ОбъектРасчетов, РасчетныйДокумент, ВИЛС_РасчетныйДокумент, ОписаниеДокумента, ВалютаДокумента", "СуммаДокумента, СуммаДолг, СуммаДолгРегл, СуммаАванс, СуммаАвансРегл");
#КонецВставки
