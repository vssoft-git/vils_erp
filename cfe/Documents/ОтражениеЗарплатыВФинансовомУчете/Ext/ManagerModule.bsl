...
#Область ТаблицаДокумента
	...
	|	ВЫБОР
	|		КОГДА НачисленнаяЗарплатаИВзносы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.ВыплатыБывшимСотрудникам)
	|			ТОГДА ЕстьNULL(СоответствияФизЛицКонтрагентам.Договор,ЗНАЧЕНИЕ(Перечисление.ТипыНалогов.ПустаяСсылка))
	|       ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыНалогов.ПустаяСсылка)
	|	КОНЕЦ как ТипНалога,
	|	ВЫБОР
	|		КОГДА НачисленнаяЗарплатаИВзносы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.ВыплатыБывшимСотрудникам)
	|			ТОГДА ЕстьNULL(СоответствияФизЛицКонтрагентам.Договор,ЗНАЧЕНИЕ(Перечисление.ВидыПлатежейВГосБюджет.ПустаяСсылка))
	...
	|       ПО  НачисленнаяЗарплатаИВзносы.ФизическоеЛицо = СоответствияФизЛицКонтрагентам.ЭлементПоиска
	//end fix Клещ А.Н. 10.05.2019
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
