...
Функция ВИЛС_ТекстОтраженияВРеглУчете()
...
#Вставка
	//begin fix Клещ А.Н. 26.06.2019 |  Строки.ФизическоеЛицо 
	|	ВЫБОР КОГДА ВтСчета.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыСПерсоналомПоОплатеТруда) ТОГДА
	|		ЕСТЬNULL(СоответствияСпособовОтражения.ЭлементСоответствия.СтатьяРасходов,Строки.ВидРабот)
	|	ИНАЧЕ 
	|		ЕСТЬNULL(СоответствияСпособовОтражения.ЭлементСоответствия.СтатьяРасходовВзносов,Строки.ВидРабот)
...
&ИзменениеИКонтроль("ЗаполнитьПодразделениеПредприятияНачисленоСдельно")
