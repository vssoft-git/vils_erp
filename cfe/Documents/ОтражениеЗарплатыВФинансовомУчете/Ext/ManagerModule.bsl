...
#Область ТекстЗапросаНачисленоСдельно
	...
	|ГДЕ
	|ОтражениеЗарплатыВФинансовомУчете.Регистратор в (Выбрать различные ДокументыКОтражению.Ссылка из ДокументыКОтражению)
	|И ОтражениеЗарплатыВФинансовомУчетеНачисленнаяЗарплатаИВзносы.Ссылка в (Выбрать различные ДокументыКОтражению.Ссылка из ДокументыКОтражению)
	|
	|СГРУППИРОВАТЬ ПО
...
#Область ТаблицаДокумента
	...
	|			ТОГДА ЕстьNULL(СоответствияФизЛицКонтрагентам.ЭлементСоответствия,ЗНАЧЕНИЕ(Справочник.ФизическиеЛица.ПустаяСсылка))
	...
	|	КОНЕЦ КАК СчетКт,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыПлатежейВГосБюджет.ПустаяСсылка) КАК ТипНалога,
	|	Максимум(ВЫБОР
	|		КОГДА НачисленнаяЗарплатаИВзносы.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоЗарплате.ВыплатыБывшимСотрудникам)
	|			ТОГДА ЕстьNULL(ДоговорыКонтрагентов.Ссылка,ЗНАЧЕНИЕ(Перечисление.ВидыПлатежейВГосБюджет.ПустаяСсылка))
	...
	//begin fix Клещ А.Н. 27.04.2019  
	|       ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ВИЛС_СоответствияПоРазделамУчета КАК СоответствияФизЛицКонтрагентам
	|       ПО  НачисленнаяЗарплатаИВзносы.ФизическоеЛицо = СоответствияФизЛицКонтрагентам.ЭлементПоиска
	|       И  СоответствияФизЛицКонтрагентам.РазрезУчета = ЗНАЧЕНИЕ(Справочник.ВИЛС_ИменаРазделовСоответствий.СоответствиеФизЛицаИКонтрагента)
	|       ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ДоговорыКонтрагентов как ДоговорыКонтрагентов
	|       ПО СоответствияФизЛицКонтрагентам.ЭлементСоответствия = ДоговорыКонтрагентов.Контрагент
	|СГРУППИРОВАТЬ ПО
	...
	|	ИНАЧЕ СтатьиРасходов.Субконто1
	|КОНЕЦ,
	|СтатьиРасходов.Субконто2,
	|СтатьиРасходов.Субконто3
	//end fix Клещ А.Н. 27.04.2019
...
Процедура ВИЛС_ЗаполнитьПодразделениеПредприятияНачисленоСдельно(НачисленнаяЗарплатаИВзносы, СтруктураПараметров)
	...
	Для Каждого Строка Из НачисленнаяЗарплатаИВзносы Цикл
		...
		Если ЗначениеЗаполнено(Строка.СпособОтраженияЗарплатыВБухучете) Тогда
			НачисленоСдельно = ОплатаСдельныхРабот[Строка.СпособОтраженияЗарплатыВБухучете] или Строка.Начисление = Перечисления.ВидыОсобыхНачисленийИУдержаний.ДоговорРаботыУслуги;
		Иначе
