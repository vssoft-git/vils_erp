...
Функция ВИЛС_ДанныеДляПечатныхФормСчетаНаОплатуИзвещения(ПараметрыПечати, МассивОбъектов)
	...
	|
	// begin fix Suetin 24.12.2019 12:13:39
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	1 														КАК НомерСтроки,
	|	ДОБАВИТЬКДАТЕ(ЗаказКлиента.Дата, ДЕНЬ, ВЫБОР
	|			КОГДА ЗаказКлиента.Договор.СрокОплаты = 0
	|				ТОГДА 10
	|			ИНАЧЕ ЗаказКлиента.Договор.СрокОплаты
	|		КОНЕЦ) 												КАК ДатаПлатежа,
	|	100 													КАК ПроцентПлатежа,
	|	ЗаказКлиента.СуммаДокумента 							КАК СуммаПлатежа,
	|	ЛОЖЬ 													КАК ЭтоЗалогЗаТару
	|ИЗ
	|	Документ.ЗаказКлиента КАК ЗаказКлиента
	|ГДЕ
	|	ЗаказКлиента.Ссылка В(&МассивОбъектов)
	|	И ЗаказКлиента.Договор.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоНакладным)
	// end fix Suetin 24.12.2019 12:13:46
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтроки,
	|	Порядок
	|;
