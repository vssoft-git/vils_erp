...
&Вместо("АктуализироватьДетальныеСведения")
&НаКлиенте
...
КонецПроцедуры
&Вместо("ПолучитьДетальныеСведенияНаСервере")
&НаСервере
Функция ВИЛС_ПолучитьДетальныеСведенияНаСервере(РеквизитыЗадачи, НеобходимоОбновлениеФормы)
...
Функция ВернутьИнформациюПОПредмету(УИДСтрокой,ВидДокумента)
	...
	Если СтрНайти(ВидДокумента,"Заявка на расходование ДС")>0  Тогда 
