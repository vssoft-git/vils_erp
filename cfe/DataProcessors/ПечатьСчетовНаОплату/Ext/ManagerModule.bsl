...
Процедура ВИЛС_ЗаполнитьТабличныйДокументСчетаНаОплату(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ПараметрыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Иначе
			...
			//begin fix Клещ А.Н. 03.06.2019  
			ВсегоКоличество = 0;
			СписокПОЕдиницам = Новый Соответствие;
			//end fix Клещ А.Н. 03.06.2019
			...
			//begin fix Клещ А.Н. 03.06.2019  
			СтруктураДанныхВсегоСкидки.Вставить("ВсегоКоличество", ВсегоКоличество);
			//end fix Клещ А.Н. 03.06.2019
			...
			Если ЕстьТоварыСчета Тогда
				СуммаОплаты = ДанныеПечати.СуммаДокумента;
				Если СоответствиеСтавокНДС.Количество() Тогда
					Для Каждого КлючИЗначение Из СоответствиеСтавокНДС Цикл
						СтавкаНДС = УчетНДСУПКлиентСервер.ПолучитьСтавкуНДС(КлючИЗначение.Ключ);
						Прервать;
					КонецЦикла;
				...
				ЗаказКлиентаОснование = ДанныеПечати.Ссылка.ДокументОснование;
				...
				|ЛЕВОЕ СОЕДИНЕНИЕ
				|	РегистрНакопления.РасчетыСКлиентами.Остатки(&ТекущаяДата,ЗаказКлиента = &РасчетныйДокумент) КАК РасчетыСКлиентамиОстатки
				|ПО
				|	ИСТИНА
				|ЛЕВОЕ СОЕДИНЕНИЕ
				...
				ПорядокРасчетов = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ЗаказКлиентаОснование, "ПорядокРасчетов");
				...
				ДанныеПечати.Договор,
				ЗаказКлиентаОснование));
				Запрос.УстановитьПараметр("ТекущаяДата",  ДанныеПечати.Ссылка.Дата);
				Выборка = Запрос.Выполнить().Выбрать();
				
				Если Выборка.Следующий() Тогда
					Аванс = ?(Выборка.СуммаОстаток<0,-Выборка.СуммаОстаток,Выборка.СуммаОстаток);
				Иначе
					Аванс = 0;
				КонецЕсли;
				...
				|////////////////////////////////////////////////////////////////////////////////
				|ВЫБРАТЬ
				|	МАКСИМУМ(ВТ_ЕдиницыИзмерения.НомерСтроки) КАК НомерСтроки,
				|	ВТ_ЕдиницыИзмерения.ЕдиницаИзмерения КАК ЕдиницаИзмерения
				|ИЗ
				|	ВТ_ЕдиницыИзмерения КАК ВТ_ЕдиницыИзмерения
				|
				|СГРУППИРОВАТЬ ПО
				|	ВТ_ЕдиницыИзмерения.ЕдиницаИзмерения
				|
				|УПОРЯДОЧИТЬ ПО
				|	НомерСтроки УБЫВ";
				Запрос.УстановитьПараметр("ВТ",ТаблицаТовары);
				Выборка = Запрос.Выполнить().Выбрать();
				
				Выборка.Следующий();
				//Сообщить(Выборка.ЕдиницаИзмерения);
			...
			//begin fix Клещ А.Н. 03.06.2019  
			ИтоговаяСтрока = ?(ЕстьТоварыСчета ,ИтоговаяСтрока+" в количестве "+ВсегоКоличество+" "+Единица,ИтоговаяСтрока);
			//end fix Клещ А.Н. 03.06.2019
			...
			Иначе
				...
				// Вывести Сумму прописью  СтруктураДанныхСуммаПрописью.Вставить("СуммаПрописью", РаботаСКурсамиВалют.СформироватьСуммуПрописью(СуммаКПрописи, ДанныеПечати.Валюта));
				//begin fix Клещ А.Н. 06.02.2019
				Если ЕстьТоварыСчета Тогда
					СуммаКОплатеЧисло = Формат(СуммаОплаты,"ЧЦ=15; ЧДЦ=2; ЧРД=.; ЧГ=3,0")+" "+ДанныеПечати.Валюта;
					СуммаСкобочки = "("+РаботаСКурсамиВалют.СформироватьСуммуПрописью(СуммаОплаты, ДанныеПечати.Валюта)+")";
					АвансЧисло = Формат(Аванс,"ЧЦ=15; ЧДЦ=2; ЧРД=.; ЧГ=3,0")+" "+ДанныеПечати.Валюта;
					АвансСкобочки = "("+Нрег(РаботаСКурсамиВалют.СформироватьСуммуПрописью(Аванс, ДанныеПечати.Валюта))+")";
					
					СтрокаДляПрописи = ?(Аванс = 0,НСтр("ru='Итого к оплате: %СуммаКОплатеЧисло% %СуммаСкобочки%.'"),НСтр("ru='Итого к оплате: %СуммаКОплатеЧисло% %СуммаСкобочки% с учетом аванса %АвансЧисло% %АвансСкобочки%'"));//,НСтр("ru='Всего наименований %Количество%, на сумму %Сумма%, с учетом аванса %Аванс%'"));//
					СтрокаДляПрописи = СтрЗаменить(СтрокаДляПрописи,"%СуммаКОплатеЧисло%",СуммаКОплатеЧисло);
					СтрокаДляПрописи = СтрЗаменить(СтрокаДляПрописи,"%СуммаСкобочки%",СуммаСкобочки);
					СтрокаДляПрописи = СтрЗаменить(СтрокаДляПрописи,"%АвансЧисло%",АвансЧисло);
					СтрокаДляПрописи = СтрЗаменить(СтрокаДляПрописи,"%АвансСкобочки%",АвансСкобочки);
					СтруктураДанныхСуммаПрописью.Вставить("СуммаПрописью", СтрокаДляПрописи);
				Иначе
