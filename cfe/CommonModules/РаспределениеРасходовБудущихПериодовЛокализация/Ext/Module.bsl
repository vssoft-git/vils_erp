...
Функция ВИЛС_ТекстОтраженияВРеглУчете() Экспорт
	...
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетКт,
	// begin fix Suetin 18.06.2020 13:05:19
	//|	КорСтроки.СтатьяРасходов КАК СубконтоКт1,
	//|	ЗНАЧЕНИЕ(Перечисление.ТипыЗатратРегл.Прочее) КАК СубконтоКт2,
	|	ВЫБОР КОГДА ТипЗначения(КорСтроки.СтатьяРасходов) = Тип(Справочник.Партнеры)
	|		ТОГДА Контрагенты.Ссылка
	|	КОГДА ТипЗначения(КорСтроки.АналитикаРасходов) = Тип(Справочник.Партнеры)
	|		ТОГДА Контрагенты.Ссылка
	|	ИНАЧЕ КорСтроки.СтатьяРасходов
	|	КОНЕЦ  КАК СубконтоКт1,
	|	ВЫБОР КОГДА ТипЗначения(КорСтроки.АналитикаРасходов) = Тип(Справочник.Партнеры)
	...
	|	ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ТипыЗатратРегл.Прочее)
	|	КОНЕЦ КАК СубконтоКт2,
	// end fix Suetin 18.06.2020 13:05:27
	|	КорСтроки.АналитикаРасходов КАК СубконтоКт3,
	|
	|	0 КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	...
	|	ПО 
	|		ОбъектыСтроительства.Ссылка = Строки.АналитикаРасходов
	// begin fix Suetin 18.06.2020 13:29:01  
	|	ЛЕВОЕ СОЕДИНЕНИЕ 
	|		Справочник.Контрагенты как Контрагенты
	|	ПО 
	|		КорСтроки.АналитикаРасходов = Контрагенты.Партнер
	// end fix Suetin 18.06.2020 13:29:07
	|ГДЕ
	|	(Строки.СуммаРегл <> 0 ИЛИ Строки.ПостояннаяРазница <> 0 ИЛИ Строки.ВременнаяРазница <> 0 ИЛИ Строки.СуммаУпр <> 0)
	|	И Строки.ВариантРаспределенияРасходовРегл <> ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаПрочиеАктивы)
	|";
