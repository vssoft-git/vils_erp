...
#КонецУдаления
#Вставка	
	//begin fix Клещ А.Н. 12.08.2019	
	//|	ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка) КАК НаправлениеДеятельностиДт, БЫЛО, СТАЛО смотри выше. Вопрос, что правильно...
...
#Область ТекстОстаткиНДСНеПринятогоКВычету        // (Дт 19.03 :: Кт 000)
	...
	"ВЫБРАТЬ // Остатки НДС не принятого к вычету (Дт 19.03 :: Кт 000) @НДСпредъявленный
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
...
#КонецОбласти
	...
	#Область ТекстПередачаВЭксплуатациюПриСписании                      // Дт 10.11.Х :: Кт 000
	ТекстПередачаВЭксплуатациюПриСписании = 
	"ВЫБРАТЬ // Остатки ТМЦ в эксплуатации (Дт 10.11.Х :: Кт 000)
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	ТекстыОтражения.Добавить(ТекстПередачаВЭксплуатациюПриСписании);
