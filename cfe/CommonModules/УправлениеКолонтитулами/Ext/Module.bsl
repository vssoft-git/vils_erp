...
Функция ВИЛС_ВернутьСкорректированныйТекстКолонтитула(Знач РезультатКон)
	МассивИсходныхСтрок = СтрРазделить(РезультатКон, Символы.ПС, Истина);
	РезультатКон = "";
	Для каждого ТекСтр Из МассивИсходныхСтрок Цикл
		...
		Иначе
			Результат = ТекСтр;	
		КонецЕсли;
		РезультатКон = ?(СтрДлина(РезультатКон) = 0, Результат, СтрШаблон("%1%2%3", РезультатКон, Символы.ПС, Результат)); 
	КонецЦикла;
	Возврат(РезультатКон);
КонецФункции // 1()
