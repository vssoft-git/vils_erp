...
Функция ВИЛС_ВернутьСтатусДокумента(СсылкаНаОбъект) Экспорт
	...
	Предыдуший = ВИЛС_ВернутьПредыдущего(СсылкаНаОбъект); 
...
Функция ВернутьСледующего(Предмет) 
...
//
//
&ИзменениеИКонтроль("ПриИзмененииСостоянияСогласования")
...
#Вставка
	...
	Если ТипЗнч(ПредметСогласования) = Тип("ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств") Тогда
		...
		Если не Объект.ВИЛС_Исполнитель = СтруктураВозврата.Исполнитель Тогда
			...
			Если не Объект.ВИЛС_КазначейскийКонтрольПройден и  СокрЛП(Объект.ВИЛС_Исполнитель) = "Казначейский контроль" Тогда
				Объект.ВИЛС_КазначейскийКонтрольПройден = Истина;
			КонецЕсли;
...
Процедура ВИЛС_ПриИзмененииУтверждения(ПредметСогласования,Состояние,ВызовИзФормыОбъекта)Экспорт
	...
	Если ТипЗнч(ПредметСогласования) = Тип("ДокументСсылка.ЗаявкаНаРасходованиеДенежныхСредств") Тогда
		...
		Следующий = ВернутьСледующего(ПредметСогласования);
		
		Если не Объект.ВИЛС_Исполнитель = Следующий Тогда
			Объект.ВИЛС_Исполнитель = Следующий;
			СтатусИзменен = Истина;
		КонецЕсли;
