...
&Вместо("ТекстРаспределениеЗатратКомиссионногоВознаграждения")
Функция ВИЛС_ТекстРаспределениеЗатратКомиссионногоВознаграждения()
	...
	//|		ТОГДА ЕСТЬNULL(СуммыДокументовВВалютеРегл.СуммаБезНДСРегл + СуммыДокументовВВалютеРегл.СуммаНДСРегл, РасчетыСКлиентами.СуммаРегл)
	//|		ИНАЧЕ ЕСТЬNULL(СуммыДокументовВВалютеРегл.СуммаБезНДСРегл, РасчетыСКлиентами.СуммаРегл - ЕСТЬNULL(НДСПредъявленный.НДС, 0)) 
	//|	КОНЕЦ КАК Сумма,
	//|	ВЫБОР 
	//|		КОГДА Операция.НалогообложениеНДС В 
	//|						(ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС),
	//|						 ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД))
	//|		ТОГДА ЕСТЬNULL(СуммыДокументовВВалютеРегл.СуммаБезНДСУпр + СуммыДокументовВВалютеРегл.СуммаНДСУпр, РасчетыСКлиентами.СуммаУпр)
	//|		ИНАЧЕ ЕСТЬNULL(СуммыДокументовВВалютеРегл.СуммаБезНДСРегл, РасчетыСКлиентами.СуммаУпр - ЕСТЬNULL(НДСПредъявленный.НДСУпр, 0)) 
	//|	КОНЕЦ СуммаУУ,
	//|	
